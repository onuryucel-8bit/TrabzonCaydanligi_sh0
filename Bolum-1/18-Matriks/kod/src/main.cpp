...
{            
    ...
    Matrix4x4 translationMatrix = Matrix4x4::makeTranslationMatrix(position);
    ...
    for (size_t i = 0; i < VertexAttributes.vertices.size(); i += 3)
    {
        ...
        //dondur
        p = rotateXMatrix.multiply(p);
        p = rotateYMatrix.multiply(p);
        p = rotateZMatrix.multiply(p);
        //tasi
    ...
    {
        Vector3 vectorA = transformedVertices[indexBufferObject[i + 0].vertex_index].toVec3();
        Vector3 vectorB = transformedVertices[indexBufferObject[i + 1].vertex_index].toVec3();
        Vector3 vectorC = transformedVertices[indexBufferObject[i + 2].vertex_index].toVec3();
