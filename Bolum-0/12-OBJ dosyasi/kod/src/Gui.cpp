...
void Gui::draw()
{
   ...
   // ImGui::Text("Ekran Boyutu (%i, %i)", screenWidth, screenHeight);
    ...
    if (ImGui::Checkbox("Arka Yuz Eleme", &cullModecb))
    {
        if (cullModecb)
        {
            m_appState.m_cullMod = CullMode::ACTIVE;
        }
        else
        {
            m_appState.m_cullMod = CullMode::NONE;
        }
    }
    ...
    ImGui::SliderFloat("pozisyon.x", &m_appState.m_position.x, -2, 2);
    ImGui::SliderFloat("pozisyon.y", &m_appState.m_position.y, -2, 2);
    ImGui::SliderFloat("pozisyon.z", &m_appState.m_position.z, -2, 2);

    if (ImGui::Button("R##1"))
    {
        m_appState.m_scale.x = 1;
        m_appState.m_scale.y = 1;
        m_appState.m_scale.z = 1;
    }
    ImGui::SliderFloat("boyut.x", &m_appState.m_scale.x, -2, 2);
    ImGui::SliderFloat("boyut.y", &m_appState.m_scale.y, -2, 2);
    ImGui::SliderFloat("boyut.z", &m_appState.m_scale.z, -2, 2);
